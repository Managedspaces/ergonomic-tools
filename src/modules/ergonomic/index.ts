/**
 * Ergonomic Risk Assessment Tool — Module Entry Point
 *
 * Wraps the existing ergonomic tool into the platform's ToolModule interface.
 */

export { CATEGORIES as ERGONOMIC_CATEGORIES, SCORABLE_QUESTIONS as ERGONOMIC_SCORABLE_QUESTIONS } from "@/lib/questionnaire";
export { COMPLIANCE_TEMPLATES as ERGONOMIC_COMPLIANCE } from "@/lib/compliance";

import { CATEGORIES } from "@/lib/questionnaire";
import { COMPLIANCE_TEMPLATES } from "@/lib/compliance";
import type { ToolModule, RiskRating } from "@/lib/shared/types";

// Re-export the existing recommendations function under the module interface
function getErgonomicRecommendations(categoryId: string, rating: RiskRating): string[] {
...
export const ERGONOMIC_TOOL: ToolModule = {
  id: "ergonomic",
  name: "Ergonomic Risk Assessment",
  description: "Evaluate your office workstation ergonomics across six dimensions of physical health.",
  icon: "Monitor",
  categories: CATEGORIES,
  getRecommendations: getErgonomicRecommendations,
  getComplianceTemplate: (countryCode: string) =>
    COMPLIANCE_TEMPLATES[countryCode.toUpperCase()] ?? null,
};
